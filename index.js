...
*POWERED BY SANDES 〽️D ㋡*`;
  ...
  // LID එකෙන් Real Phone Number එක සෙවීමේ Async Helper Function එක
const getPhoneNumber = async (jid, conn, currentChat) => {
    if (!jid) return 'Unknown';
    try {
        let rawNum = jid.split('@')[0].split(':')[0].replace(/[^0-9]/g, '');

        // 1. LID එකක් නෙමේ නම් ඍජුවම number එක දෙන්න
        if (!jid.endsWith('@lid') && rawNum.length <= 12) {
            return rawNum;
        }

        // 2. Chat එක Group එකක් නම් Group Participants වලින් Real JID එක හොයමු
        if (currentChat && currentChat.endsWith('@g.us')) {
            try {
                const groupMetadata = await conn.groupMetadata(currentChat);
                ...
                // Metadata ගන්න බැරි වුණොත් pass කරන්න
            }
        }

        // 3. Signal Repository / Contacts වලින් Check කිරීම
        if (conn?.signalRepository?.jidToUser) {
            const userJid = await conn.signalRepository.jidToUser(jid);
            if (userJid) return userJid.split('@')[0].split(':')[0].replace(/[^0-9]/g, '');
        }

        // 4. Contacts store එකෙන් check කිරීම
        const contact = conn?.contacts?.[jid] || conn?.chats?.[jid];
...
if (mek.message && mek.message.protocolMessage && mek.message.protocolMessage.type === 0) {
    ...
    const myBotNum = conn?.user?.id ? conn.user.id.split('@')[0].split(':')[0].replace(/[^0-9]/g, '') : '';
    ...
    if (savedMsg) {
        ...
        let deleterRaw = mek.key.participant || mek.key.remoteJid;
        
        let deleterNum = await getPhoneNumber(deleterRaw, conn, currentChat);
        
        const ownerNumbers = ['94787518010']; 
        const ownerLids = ["123017090887835@lid", "183150860841183@lid"]; 

        const isOwnerDeleter = ownerNumbers.includes(deleterNum) || ownerLids.includes(deleterRaw);

        if (deleterNum === myBotNum || isOwnerDeleter) return;
        ...
        let senderRaw = savedMsg.key.participant || savedMsg.key.remoteJid;
        let senderNum = await getPhoneNumber(senderRaw, conn, currentChat);
        ...
        detailsText += `👤 *_Sent By :_* +${senderNum}\n`;
        detailsText += `🗑 *_Deleted By :_* +${deleterNum}\n`;
        detailsText += `🕒 *_Time :_* ${msgTime}\n\n`
        detailsText += `> *POWERED BY SANDES 〽️D ㋡*`;
